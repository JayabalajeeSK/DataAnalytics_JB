...
select * from dbo.Sales
select max(saledate) [Max SalesDate] from dbo.Sales -- by Latest Date

select * from dbo.Sales
select max(paymentmethod) [Max Pay method] from dbo.Sales --by A to Z 

select * from dbo.Sales
--Maximum Quantity sold for each productID -- for each products - we using product_id as GROUP BY
select productid,max(quantity) [Maximum Quantity] from dbo.Sales
group by productid

select * from dbo.Sales
--Maximum Total Amount for all distinct dates in saledate column -- for each Date for ALL DISTINCT Dates - we using SalesDate as GROUP BY
